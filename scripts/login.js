...
/* ─────────────────────────────────────────
   ADMIN WHITELIST
   Add authorised admin emails here.
   Checked after Firebase auth succeeds.
───────────────────────────────────────── */
const ADMIN_EMAILS = [
  "admin@coephaltan.edu.in",
...
document.querySelector(".btn-login").addEventListener("click", async function (e) {
  ...
  // Read current role from inline script
  var role = (typeof currentRole !== "undefined") ? currentRole : "teacher";
  ...
  btn.textContent  = role === "admin" ? "Authenticating…" : "Logging in…";
  btn.disabled     = true;
  ...
  try {
    ...
    if (role === "admin") {
      if (!ADMIN_EMAILS.includes(user.email.toLowerCase())) {
        await auth.signOut();
        console.warn("[Admin] Unauthorised login attempt:", user.email);
